...
send_headers (lsquic_stream_ctx_t *st_h)
{
    ...
    hbuf.off = 0;
    struct lsxpack_header headers_arr[12];
#define V(v) (v), strlen(v)
    ...
    if (0 == strcmp(st_h->client_ctx->method, "CONNECT")) {
        st_h->client_ctx->protocol = "connect-udp";
        header_set_ptr(&headers_arr[h_idx++], &hbuf, V(":protocol"), V(st_h->client_ctx->protocol));
    }
